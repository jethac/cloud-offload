...
def test_loopback_coordinator_has_one_submit_and_closure(tmp_path):
    ...
    job_id, replay = store.submit(plan=value, preflight_id="pf", candidate_id="c", key="request-1", job_id="job-1")
    assert job_id == "job-1" and replay is None
    replay_id, _ = store.submit(plan=value, preflight_id="pf", candidate_id="c", key="request-1", job_id="job-2")
...
def test_cancellation_and_unknown_submit_are_monotonic(tmp_path):
    ...
    value = plan()
    store.preflight(value, {"preflight_id": "pf", "status": "ready", "expires_at": "2999-01-01T00:00:00Z", "candidates": [{"candidate_id": "c"}]})
    store.submit(plan=value, preflight_id="pf", candidate_id="c", key="request-1", job_id="job-1")
